...
import { ILoadResult, IRunStrategyResult, RunStrategy } from './RunStrategy';
...
export class AuthenticatedStrategy extends RunStrategy {
   ...
   * @private
   */
  private static _loadJobs = async (page: Page, jobsTot: number, timeout = 2000): Promise<any> => {
   ...
   * @private
   */
  private static _loadJobDetails = async (page: Page, jobId: string, timeout = 2000): Promise<ILoadResult> => {
    const pollingTime = 50;
    let elapsed = 0;
    let loaded = false;
    await sleep(pollingTime);
   ...
   * @private
   */
  private static _paginate = async (page: Page, tag: string, paginationSize = 25, timeout = 2000): Promise<ILoadResult> => {
   ...
   * @param {string} tag
   */
  private static _hideChatPanel = async (page: Page, tag: string): Promise<void> => {
    try {
   ...
   * @param {string} tag
   */
  private static _acceptCookies = async (page: Page, tag: string): Promise<void> => {
    try {
      await page.evaluate(() => {
   ...
   * @param tag
   */
  private static _acceptPrivacy = async (page: Page, tag: string): Promise<void> => {
    try {
      await page.evaluate((selector) => {
        const privacyButton = Array.from(document.querySelectorAll<HTMLElement>(selector)).find((e) => e.innerText === 'Accept');
  ...
  public run = async (
    ...
    const options = query.options || { limit: 100 };
    ...
    while (metrics.processed < limit) {
      ...
      while (jobIndex < jobsTot && metrics.processed < limit) {
        ...
        const donePass = async () => {
          jobIndex += 1;
          metrics.processed += 1;
          logger.info(tag, `Processed`);

          // Try fetching more jobs
          if (metrics.processed < limit && jobIndex === jobsTot && jobsTot < paginationSize) {
        ...
        try {
          await summery.updateDetails(page);
          // Try to load job details and extract job link
          logger.debug(tag, 'Evaluating selectors', [selectors.jobs]);
          const loadDetailsResult = await AuthenticatedStrategy._loadJobDetails(page, summery.jobId);
          ...
          if (options.applyLink) {
            ...
            if (applyLinkRes.success) {
              summery.applyLink = applyLinkRes.url as string;
            }
          }
        } catch (err) {
          jobIndex++;
          const errorMessage = `${tag}\t${(err as Error).message}`;
        ...
        this.scraper.emit(events.scraper.data, summery);
        await donePass();
      }
