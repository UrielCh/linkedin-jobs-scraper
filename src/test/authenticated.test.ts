...
import { events, experienceLevelFilter, LinkedinScraper, onSiteOrRemoteFilter, relevanceFilter, timeFilter } from '../index';
import { JobSummery } from '../scraper/strategies/JobSummery';

describe('[TEST]', () => {
  jest.setTimeout(240000);

  const onDataFn = (data: JobSummery): void => {
  ...
  const descriptionFn = () => (document.querySelector<HTMLElement>('.jobs-description') || { innerText: '' }).innerText.replace(/[\s\n\r]+/g, ' ').trim();
